...
func (c *DeviceController) ListDevices() {
	...
	devices, err := q.Device.Preload(q.Device.Site, q.Device.ValueStream).Find()
	if err != nil {
	...
	c.TplName = "devices/index.html"
}

// NewDevice renders the create device form (Web)
func (c *DeviceController) NewDevice() {
	userID := c.GetSession("user_id")
	if userID == nil {
		c.Redirect("/login", 302)
		return
	...
	sites, err := dal.Site.Find()
	if err != nil {
		c.Data["Error"] = "Failed to load sites"
	}

	valueStreams, err := dal.ValueStream.Find()
	if err != nil {
		c.Data["Error"] = "Failed to load value streams"
	}
...
// EditDevice renders the edit device form (Web)
func (c *DeviceController) EditDevice() {
	userID := c.GetSession("user_id")
	if userID == nil {
		c.Redirect("/login", 302)
		return
	...
	device, err := dal.Device.Where(dal.Device.ID.Eq(uint(id))).First()
	if err != nil {
...
// ViewDevice renders the view device page (Web)
func (c *DeviceController) ViewDevice() {
	userID := c.GetSession("user_id")
	if userID == nil {
		c.Redirect("/login", 302)
		return
