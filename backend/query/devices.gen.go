...
func newDevice(db *gorm.DB, opts ...gen.DOOption) device {
	...
	_device.Type = field.NewString(tableName, "type")
...
type IDeviceDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.Device, err error)
	GetByID(id int) (result model.Device, err error)
	GetByRoles(rolesName []string) (result []*model.Device, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (d deviceDo) FilterWithNameAndRole(name string, role string) (result []model.Device, err error) {
...
func (d deviceDo) GetByRoles(rolesName []string) (result []*model.Device, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, rolesName)
	generateSQL.WriteString("SELECT * FROM devices WHERE role IN ? ")
